pipeline {
    ...
    stages {
        stage('Checkout Stage') {
            steps {
            //git credentialsId: 'bitbucket', url: 'https://EnExSe@bitbucket.org/enexse/ees-intranet-ms-eureka-server.git', branch: 'feature/dockerization'
            checkout([$class: 'GitSCM',
                            branches: [[name: '*/main']],
                            extensions: [],
                            userRemoteConfigs: [[credentialsId: 'git',
                            url: 'https://github.com/Enexse/ees-intranet-ms-eureka-server.git']]])
